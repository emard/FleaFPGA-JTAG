...
* 4/25/06 ht   Change some variables from uint16_t or int32_t
*              to long int32_t to make the code compiler independent.
...
uint16_t g_usFlowControl	= 0x0000;
uint16_t g_usDataType		= 0x0000;
...
uint8_t g_ucEndDR = DRPAUSE;
uint8_t g_ucEndIR = IRPAUSE;
...
uint16_t g_usHeadDR = 0;
uint16_t g_usHeadIR = 0;
uint16_t g_usTailDR = 0;
uint16_t g_usTailIR = 0;
...
***************************************************************/
//08/28/08 NN Added Calculate checksum support.
uint32_t g_usChecksum = 0;
uint32_t g_uiChecksumIndex = 0;
...
uint8_t * g_pucHeapMemory		= NULL;
uint16_t g_iHeapCounter		= 0;
uint16_t g_iHEAPSize          = 0;
...
uint16_t g_usIntelDataIndex	= 0;
uint16_t g_usIntelBufferSize	= 0;
...
uint16_t g_usTDOSize      = 0;
uint16_t g_usMASKSize		= 0;
uint16_t g_usTDISize		= 0;
uint16_t g_usDMASKSize    = 0;
uint16_t g_usLCOUNTSize   = 0;
uint16_t g_usHDRSize		= 0;
uint16_t g_usTDRSize		= 0;
uint16_t g_usHIRSize		= 0;
uint16_t g_usTIRSize		= 0;
uint16_t g_usHeapSize		= 0;
uint8_t g_usCalculateChecksum = 0x00;
...
struct {
	 ...
	 uint8_t  Pattern;   /* The tragetory of TMS */
...
int8_t ispVMCode();
int8_t ispVMDataCode();
int32_t ispVMDataSize();
void ispVMData( uint8_t * Data );
int8_t ispVMShift( int8_t Code );
int8_t ispVMAmble( int8_t Code );
int8_t ispVMLoop( uint16_t a_usLoopCount );
int8_t ispVMBitShift( int8_t mode, uint16_t bits );
void ispVMComment( uint16_t a_usCommentSize );
void ispVMHeader( uint16_t a_usHeaderSize );
int8_t ispVMLCOUNT( uint16_t a_usCountSize );
void ispVMClocks( uint16_t Clocks );
void ispVMBypass( int8_t ScanType, uint16_t Bits );
void ispVMStateMachine( int8_t NextState );
void ispVMStart();
void ispVMEnd();
int8_t ispVMSend(uint16_t);
int8_t ispVMRead(uint16_t);
int8_t ispVMReadandSave(uint16_t);
int8_t ispVMProcessLVDS( uint16_t a_usLVDSCount );
...
*
***************************************************************/
extern void vme_out_char(uint8_t charOut);
extern void vme_out_hex(uint8_t hexOut);
extern void vme_out_string(char *stringOut);
extern uint8_t GetByte();
extern void ispVMMemManager( int8_t types, uint16_t size );
...
*
***************************************************************/
extern void ispVMDelay( uint16_t a_usMicroSecondDelay );
extern uint8_t readPort();
extern void writePort( uint8_t pins, uint8_t value );
extern void sclock();
extern int8_t g_cCurrentJTAGState;
extern const uint8_t g_ucPinTDI;
extern const uint8_t g_ucPinTCK;
extern const uint8_t g_ucPinTMS;
extern const uint8_t g_ucPinENABLE;
...
const char * GetState( uint8_t a_ucState )
{
	switch( a_ucState ) {
	case RESET:
		return( "RESET" );
...
void PrintData( uint16_t a_iDataSize, uint8_t * a_pucData )
{
	//09/11/07 NN added local variables initialization
	uint16_t usByteSize  = 0;
	uint16_t usBitIndex  = 0;
	signed short usByteIndex   = 0;
	uint8_t ucByte       = 0;
	uint8_t ucFlipByte   = 0;

	if ( a_iDataSize % 8 ) {
		//09/11/07 NN Type cast mismatch variables
		usByteSize = (uint16_t)(a_iDataSize / 8 + 1);
	}
	else {
		//09/11/07 NN Type cast mismatch variables
		usByteSize = (uint16_t)(a_iDataSize / 8);
	}
	printf( "(" );
	//09/11/07 NN Type cast mismatch variables
...
int32_t ispVMDataSize()
{
	//09/11/07 NN added local variables initialization
	int32_t iSize           = 0;
	int8_t cCurrentByte = 0;
	int8_t cIndex       = 0;
	cIndex = 0;
	while ( ( cCurrentByte = GetByte() ) & 0x80 ) {
		iSize |= ( ( int32_t ) ( cCurrentByte & 0x7F ) ) << cIndex;
		cIndex += 7;
	}
	iSize |= ( ( int32_t ) ( cCurrentByte & 0x7F ) ) << cIndex;
	return iSize;
	...
	while ( ( cOpcode = GetByte() ) >= 0 ) {
		...
		case WAIT:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			usDelay = (uint16_t) ispVMDataSize();
			ispVMDelay( usDelay );
		...
		case TCK:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			usToggle = (uint16_t) ispVMDataSize();
		...
		case SETFLOW:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			g_usFlowControl |= (uint16_t) ispVMDataSize();
			break;
		case RESETFLOW:
		...
		case HEAP:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			g_iHEAPSize = (uint16_t) ispVMDataSize();
			...
			*****************************************************************************/
			...
			ispVMMemManager( HEAP, ( uint16_t ) g_iHEAPSize );
			break;
		case REPEAT:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			iRepeatSize = (uint16_t) ispVMDataSize();
			
			cRetCode = ispVMLoop( ( uint16_t ) iRepeatSize );
			if ( cRetCode != 0 ) {
				return ( cRetCode );
			}
			break;
		...
		case SHR:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			g_usShiftValue = (uint16_t) (g_usRepeatLoops * (uint16_t)GetByte());
			break;
		case SHL:
		...
		case FREQUENCY:
			...
			***************************************************************/
			...
			//09/11/07 NN Type cast mismatch variables
			g_iFrequency = (int32_t) (ispVMDataSize() );
		...
		case LCOUNT:
			...
			***************************************************************/
			...
			cRetCode = ispVMLCOUNT( ( uint16_t ) ispVMDataSize() );
			if ( cRetCode != 0 ) {
				return ( cRetCode );
			}
			break;
		...
		case COMMENT:
			...
			***************************************************************/
			...
			ispVMComment( ( uint16_t ) ispVMDataSize() );
			break;
		case LVDS:
			...
			***************************************************************/
			...
			ispVMProcessLVDS( ( uint16_t ) ispVMDataSize() );
			break;
		case HEADER:
			...
			***************************************************************/
			...
			ispVMHeader( ( uint16_t ) ispVMDataSize() );
			break;
...
int8_t ispVMDataCode()
{
	//09/11/07 NN added local variables initialization
	int8_t cDataByte    = 0;
	int8_t siDataSource = 0;  /*source of data from file by default*/
...
void ispVMData( uint8_t * ByteData )
{
	//09/11/07 NN added local variables initialization
	uint16_t size               = 0;
	uint16_t i, j, m, getData   = 0;
	uint8_t cDataByte           = 0;
	uint8_t compress            = 0;
	uint16_t FFcount            = 0;
	uint8_t compr_char          = 0xFF; 
	uint16_t index              = 0;
	...
	if (g_usiDataSize%8>0) {
		//09/11/07 NN Type cast mismatch variables
		size = (uint16_t)(g_usiDataSize/8 + 1);
	}
	else {
		//09/11/07 NN Type cast mismatch variables
		size = (uint16_t)(g_usiDataSize/8);
	}
		...
		default:   
			...
			for (index = 0; index < compress; index++) {
				...
				for (i = 0; i < size*2/compress; i++){
					//09/11/07 NN Type cast mismatch variables
					j = (uint16_t)(index + (i*(uint16_t)compress));
	...
	for (index = 0; index < size; index++) {
		if (FFcount <= 0) {
			...
			if ((compression) &&(cDataByte == compr_char)) /*decompression is on*/
				//09/11/07 NN Type cast mismatch variables
				FFcount = (uint16_t) ispVMDataSize();     /*The number of 0xFF or 0x00 bytes*/
		}
		else {
...
int8_t ispVMShift( int8_t a_cCode )
{
	//09/11/07 NN added local variables initialization
	uint16_t iDataIndex  = 0;
	uint16_t iReadLoop   = 0;
	int8_t cRetCode       = 0;
...
int8_t ispVMLoop(uint16_t a_usLoopCount)
{
	//09/11/07 NN added local variables initialization
	int8_t cRetCode      = 0;
	uint16_t iHeapIndex = 0;
	uint16_t iLoopIndex = 0;
...
void ispVMCalculateCRC32( uint8_t a_ucData ) 
{
	//09/11/07 NN added local variables initialization
	uint8_t ucIndex          = 0;
	uint8_t ucFlipData       = 0;
	uint16_t usCRCTableEntry = 0;
	...
	//09/11/07 NN Type cast mismatch variables
	usCRCTableEntry = (uint16_t)(crc_table[ g_usCalculatedCRC & 0xF ]);
	g_usCalculatedCRC = (uint16_t)(( g_usCalculatedCRC >> 4 ) & 0x0FFF);
	g_usCalculatedCRC = (uint16_t)(g_usCalculatedCRC ^ usCRCTableEntry ^ crc_table[ ucFlipData & 0xF ]);
	usCRCTableEntry = (uint16_t)(crc_table[ g_usCalculatedCRC & 0xF ]);
	g_usCalculatedCRC = (uint16_t)(( g_usCalculatedCRC >> 4 ) & 0x0FFF);
	g_usCalculatedCRC = (uint16_t)(g_usCalculatedCRC ^ usCRCTableEntry ^ crc_table[ ( ucFlipData >> 4 ) & 0xF ]);
}
...
int8_t ispVMLCOUNT( uint16_t a_usCountSize )
{
	uint16_t usContinue		  = 1;
	uint16_t usIntelBufferIndex = 0;
	uint16_t usCountIndex       = 0;
	int8_t cRetCode              = 0;
	int8_t cRepeatHeap           = 0;
	int8_t cOpcode               = 0;
	uint8_t ucState             = 0;
	uint16_t usDelay            = 0;
	uint16_t usToggle           = 0;

	g_usIntelBufferSize = (uint16_t)ispVMDataSize();
...
void ispVMBypass( int8_t ScanType, uint16_t Bits )
{
	//09/11/07 NN added local variables initialization
	uint16_t iIndex       = 0;
	uint16_t iSourceIndex = 0;
	uint8_t cBitState     = 0;
	uint8_t cCurByte      = 0;   
	uint8_t * pcSource    = NULL;
	...
	if(pcSource)
	{
		...
		for ( iIndex = 0; iIndex < Bits - 1; iIndex++ ) {
			...
			cBitState = ( uint8_t ) ( ( ( cCurByte << iIndex % 8 ) & 0x80 ) ? 0x01 : 0x00 );
...
void ispVMStateMachine( int8_t cNextJTAGState )
{
	//09/11/07 NN added local variables initialization
	int8_t cPathIndex  = 0;
	int8_t cStateIndex = 0;
	int8_t found       = 0;
	...
	if(found)
	{
		...
		for ( cPathIndex = 0; cPathIndex < g_JTAGTransistions[ cStateIndex ].Pulses; cPathIndex++ ) {
			if ( ( g_JTAGTransistions[ cStateIndex ].Pattern << cPathIndex ) & 0x80 ) {
				writePort( g_ucPinTMS, ( uint8_t ) 0x01 );
			}
			else {
				writePort( g_ucPinTMS, ( uint8_t ) 0x00 );
			}
			sclock();
		}
...
int8_t ispVMSend( uint16_t a_usiDataSize )
{
	//09/11/07 NN added local variables initialization
	uint16_t iIndex       = 0;
	uint16_t iInDataIndex = 0;
	uint8_t cCurByte      = 0;
	uint8_t cBitState     = 0;
...
int8_t ispVMRead( uint16_t a_usiDataSize )
{
	//09/11/07 NN added local variables initialization
	uint16_t usDataSizeIndex    = 0;
	uint16_t usErrorCount       = 0;
	uint16_t usLastBitIndex     = 0;
	uint8_t cDataByte           = 0;
	uint8_t cMaskByte           = 0;
	uint8_t cInDataByte         = 0;
	uint8_t cCurBit             = 0;
	uint8_t cByteIndex          = 0;
	uint16_t usBufferIndex      = 0;
	uint8_t ucBitTested			= 0;
	uint8_t ucDisplayByte       = 0x00;
	uint8_t ucDisplayFlag       = 0x01;
	char StrChecksum[256]            = {0};

	//09/11/07 NN Type cast mismatch variables
	usLastBitIndex = (uint16_t)(a_usiDataSize - 1);
	...
	*****************************************************************************/
	...
	for ( usDataSizeIndex = 0; usDataSizeIndex < a_usiDataSize; usDataSizeIndex++ ) { 
		...
		*****************************************************************************/
		...
		if ( g_usDataType & TDO_DATA ) {
			...
			else
			{
				if ( ( ( ( cMaskByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 ) ) {
					ucBitTested = 1;
					if ( cCurBit != ( uint8_t ) ( ( ( cDataByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 ) ) {
		...
		if (gVerbose && gJTAGMode == JTAG_NONE)
		{
			if (ucBitTested)
				gColumn += printf("=%1X", ( uint8_t ) ( ( ( cDataByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 )); 
			else
				gColumn += printf("=X"); 
		...
		*****************************************************************************/
		...
		writePort( g_ucPinTDI, ( uint8_t ) ( ( ( cInDataByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 ) );
...
//		vme_out_string( "Display Data: 0x" );
		...
		//09/11/07 NN Type cast mismatch variables
		for ( usDataSizeIndex = (uint16_t)( ( a_usiDataSize + 7 ) / 8 ); usDataSizeIndex > 0 ; usDataSizeIndex-- ) {
...
int8_t ispVMReadandSave( uint16_t a_usiDataSize )
{
	//09/11/07 NN added local variables initialization
	uint16_t	usDataSizeIndex = 0;
	uint16_t	usLastBitIndex  = 0;
	uint16_t	usBufferIndex   = 0;
	uint16_t	usOutBitIndex   = 0;
	uint16_t	usLVDSIndex     = 0;
	uint8_t		cDataByte       = 0;
	uint8_t		cDMASKByte      = 0;
	uint8_t		cInDataByte     = 0;
	uint8_t		cCurBit         = 0;
	uint8_t		cByteIndex      = 0;
	int8_t		cLVDSByteIndex  = 0;
	...
	***************************************************************/
	...
	for ( usDataSizeIndex = 0; usDataSizeIndex < a_usiDataSize; usDataSizeIndex++ ) {
		...
		cCurBit = readPort();
		cDataByte = ( uint8_t ) ( ( ( cInDataByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 );
		...
		***************************************************************/
		...
		if ( ( ( ( cDMASKByte << cByteIndex ) & 0x80 ) ? 0x01 : 0x00 ) ) {
			...
			***************************************************************/
			...
			g_pucOutData[ usOutBitIndex / 8 ] |= ( uint8_t ) ( ( ( cDataByte & 0x1 ) ? 0x01 : 0x00 ) << ( 7 - usOutBitIndex % 8 ) );
		}
		else {
			...
			***************************************************************/
			...
			g_pucOutData[ usOutBitIndex / 8 ] |= ( uint8_t ) ( ( ( cCurBit & 0x1 ) ? 0x01 : 0x00 ) << ( 7 - usOutBitIndex % 8 ) );
		}
		...
		for ( usLVDSIndex = 0; usLVDSIndex < g_usLVDSPairCount; usLVDSIndex++ ) {
			if ( g_pLVDSList[ usLVDSIndex ].ucUpdate ) {
				...
				***************************************************************/
				...
				cDataByte = ( uint8_t ) ( ( ( g_pucOutData[ g_pLVDSList[ usLVDSIndex ].usPositiveIndex / 8 ] << ( g_pLVDSList[ usLVDSIndex ].usPositiveIndex % 8 ) ) & 0x80 ) ? 0x01 : 0x00 );
				//09/11/07 NN Type cast mismatch variables
				cDataByte = ( uint8_t ) (!cDataByte);
	...
	***************************************************************/
	...
	for ( usLVDSIndex = 0; usLVDSIndex < g_usLVDSPairCount; usLVDSIndex++ ) {
		...
		***************************************************************/
		...
		//09/11/07 NN Type cast mismatch variables
		g_pLVDSList[ usLVDSIndex ].usPositiveIndex = (uint16_t) ispVMDataSize();
		//09/11/07 NN Type cast mismatch variables
